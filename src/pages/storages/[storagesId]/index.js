...
const StoragesDetailsPage = ({ processor }) => {
  return (
    <div>
      <div>
        <Row>
...
export default StoragesDetailsPage;
...
export const getStaticPaths = async () => {
  ...
  const paths = processors.data.map((processor) => ({
    params: { storagesId: processor._id },
  }));
...
export const getStaticProps = async (context) => {
  ...
  const res = await fetch(
    `https://backend-six-topaz.vercel.app/api/v1/storages/${params.storagesId}`
  );
  const data = await res.json();
  return { props: { processor: data } };
};

StoragesDetailsPage.getLayout = function getLayout(page) {
  return (
    <Layout>
